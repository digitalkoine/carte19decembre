...
countries <- st_read('data/shp/shp.shp')

# Read the CSV file containing pizzeria data
point <- read.csv("data/data.csv")
...
map <- leaflet(countries) %>% 
  ...
  addMarkers(data = point,
             ...
             lat = ~lat,
             group = "La classe",
             popup = ~paste("<b>", name, "</b>", " a efféctué  ", "<b>", weight, "</b>", " voyages !",
                            sep = " ")) %>%
  ...
  addPolygons(data = countries, 
              ...
              smoothFactor = 0,
              group = "Countries",
              label = ~paste("La raison de ces", countries$weight, " voyages est le ", countries$label)) %>%
